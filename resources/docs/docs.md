...
The `home-routes` handler in the `cause-agent.routes.home` namespace
...
The `home-page` function will in turn call the `cause-agent.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `cause-agent.handler` namespace:
...
Request middleware functions are located under the `cause_agent.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `cause-agent.dev-middleware` namespace found in
